...
export function useCart() {
	...
	async function request<T>(
		...
		const { data, errors } = await storefront.request<T>(operation, {
			variables,
		});
		if (errors) {
			throw new Error(
				Array.isArray(errors) ? errors[0]?.message : String(errors),
			);
		}
		return data;
	}
	...
	async function refresh() {
		...
		try {
			const data = await request<{ cart: Cart | null }>(CART_QUERY, {
				cartId: cartId.value,
	...
	async function addLine(
		...
		quantity: number,
		gift: GiftBoxSchema | null = null,
	) {
		loading.value = true;
		try {
			const lines = [
				{ merchandiseId, quantity, attributes: giftAttributes(gift) },
			];
			const ctx = { language: language.value, country: country.value };
			...
			if (!cartId.value) {
				...
				if (!data?.cartLinesAdd?.cart) {
					cartId.value = null;
					const created = await request<{ cartCreate: CartMutationResult }>(
						CART_CREATE,
						{ lines, ...ctx },
					);
